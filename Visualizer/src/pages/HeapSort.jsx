...
import DeletionAnimation from "../components/heapSortComponents/DeletionAnimation";
...
export default function HeapSort() {
  ...
  function goClick() {
    ...
    if (!ready) {
      const heapArr = arrToHeap(
        arr,
        selectRef.current.value === "asc" ? "min" : "max"
      );
      const treeArr = heapArrToTreeArr(heapArr);
      const treeObj = heapArrToTreeObj(heapArr);
      dispatch(bstActions.setHeapArr([...heapArr]));
      dispatch(
        bstActions.setHeapType(
          selectRef.current.value === "asc" ? "min" : "max"
        )
      );
      dispatch(bstActions.setTreeArr(JSON.parse(JSON.stringify(treeArr))));
      dispatch(bstActions.setTreeObject(JSON.parse(JSON.stringify(treeObj))));
    }
          ...
          <div className="flex  border-2 text-[#c08552]   border-neutral-200">
            <div className="flex flex-col max-w-[300px]   ">
              ...
              <p className="text-sm p-1 m-1 px-3 bg-[#f3e9dc] border-2 border-[#c08552] rounded-xl">
                Enter upto 20 numbers from 0 to 1000 separated by space. The
